...
const CLUES = {
  ...
  canteen: { icon: '◒', title: 'Metal canteen', text: 'A rusty water bottle lies beside the fresh footprints.', question: 'Did it belong to the people who once hid here?', image: IMAGES.scene4, imageAlt: 'A dented, rusty metal canteen resting against the tunnel wall.', caption: 'Rusty canteen beside the fresh prints' },
  bcs: { icon: 'B·C·S', title: 'B.C.S. initials', text: 'The initials are carved beside an old arrow. The carving predates the school.', question: 'Whose name do the letters hide?', image: IMAGES.scene4, imageAlt: 'The rough old tunnel wall where the initials B.C.S. and an arrow were carved.', caption: 'Old initials carved beside an arrow' },
  photograph: { icon: '▣', title: 'Wartime photograph', text: 'Several families stand at a cave entrance. A man holds an old lantern.', question: 'Were they hiding—or building something here?', image: IMAGES.scene5, imageAlt: 'Kumar examining a yellowed wartime photograph found inside an old metal box.', caption: 'Wartime photograph found in the cave' },
  map: { icon: '⌁', title: 'Cave map', text: 'A hand-drawn wartime map shows a second tunnel and a ventilation passage.', question: 'Which route can still be trusted?', image: IMAGES.scene7, imageAlt: 'Mei Lin pointing to a route on a torn hand-drawn cave map.', caption: 'Hand-drawn map of the cave passages' },
  lantern: { icon: '♢', title: 'Warm lantern', text: 'The lantern looks decades old, but its metal side is warm.', question: 'Who lit it?', image: IMAGES.scene6, imageAlt: 'An old metal lantern glowing warmly on top of the cave map.', caption: 'The old lantern is still glowing' },
  badge: { icon: '⬟', title: 'School badge', text: 'A clean Bukit Cempaka badge lies among objects that are decades old.', question: 'Who from the school came here recently?', image: IMAGES.scene7, imageAlt: 'Kumar comparing a clean school badge with the badge on his uniform.', caption: 'A clean badge among decades-old objects' },
  records: { icon: '▤', title: 'Hidden school records', text: 'Names, drawings and lessons from Sekolah Rakyat Bukit Cempaka, 1944.', question: 'How many stories are still waiting underground?', image: IMAGES.scene5, imageAlt: 'Old document boxes and papers stored in the hidden cave chamber.', caption: 'Records preserved inside wartime boxes' },
};
...
function cluePanel(id) {
  ...
  const visual = clue.image
    ? `<figure class="clue-evidence-image clue-evidence-${id}"><img src="${clue.image}" alt="${escapeHtml(clue.imageAlt)}">${id === 'bcs' ? '<div class="clue-etching" aria-hidden="true"><span>→</span><b>B · C · S</b></div>' : ''}<figcaption>${renderClueMark(id)}<span>${escapeHtml(clue.caption)}</span></figcaption></figure>`
...
function renderClueMark(id) {
  ...
  if (id === 'canteen') return `<svg class="clue-mark object-mark" viewBox="0 0 64 64" aria-hidden="true" focusable="false"><path d="M25 7h14v8l5 7v33H20V22l5-7V7Z" fill="none" stroke="currentColor" stroke-width="4" stroke-linejoin="round"/><path d="M26 29h12M26 36h12" stroke="currentColor" stroke-width="3" stroke-linecap="round"/></svg>`;
  if (id === 'bcs') return `<svg class="clue-mark initials-mark" viewBox="0 0 82 50" aria-hidden="true" focusable="false"><path d="M5 38 28 13M18 13h10v10" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"/><text x="32" y="35" fill="currentColor" font-size="18" font-weight="800">BCS</text></svg>`;
  if (id === 'photograph') return `<svg class="clue-mark object-mark" viewBox="0 0 64 64" aria-hidden="true" focusable="false"><rect x="8" y="11" width="48" height="42" fill="none" stroke="currentColor" stroke-width="4"/><circle cx="42" cy="25" r="5" fill="currentColor"/><path d="m13 47 14-15 9 9 6-6 10 12" fill="none" stroke="currentColor" stroke-width="3" stroke-linejoin="round"/></svg>`;
  if (id === 'map') return `<svg class="clue-mark object-mark" viewBox="0 0 64 64" aria-hidden="true" focusable="false"><path d="m6 13 17-6 18 7 17-6v43l-17 6-18-7-17 6V13Z" fill="none" stroke="currentColor" stroke-width="3" stroke-linejoin="round"/><path d="M23 8v42M41 14v42M13 38c9-17 25 9 38-12" fill="none" stroke="currentColor" stroke-width="3" stroke-dasharray="4 4"/></svg>`;
  if (id === 'lantern') return `<svg class="clue-mark object-mark" viewBox="0 0 64 64" aria-hidden="true" focusable="false"><path d="M22 18h20l5 35H17l5-35Z" fill="none" stroke="currentColor" stroke-width="3"/><path d="M25 18c0-13 14-13 14 0M20 27h24M20 46h24" fill="none" stroke="currentColor" stroke-width="3"/><path d="m25 28 14 18m0-18L25 46" stroke="currentColor" stroke-width="2"/></svg>`;
  if (id === 'badge') return `<svg class="clue-mark object-mark" viewBox="0 0 64 64" aria-hidden="true" focusable="false"><path d="M10 9h44v21c0 14-9 22-22 27C19 52 10 44 10 30V9Z" fill="none" stroke="currentColor" stroke-width="4" stroke-linejoin="round"/><path d="m32 18 4 8 9 1-7 6 2 9-8-5-8 5 2-9-7-6 9-1 4-8Z" fill="currentColor"/></svg>`;
  if (id === 'records') return `<svg class="clue-mark object-mark" viewBox="0 0 64 64" aria-hidden="true" focusable="false"><path d="M14 7h29l8 8v42H14V7Z" fill="none" stroke="currentColor" stroke-width="3" stroke-linejoin="round"/><path d="M43 7v10h8M22 27h21M22 36h21M22 45h15" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"/></svg>`;
